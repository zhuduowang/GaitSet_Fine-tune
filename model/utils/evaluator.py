        ...
        for gallery_seq in gallery_seq_dict[dataset]:
            for (v1, probe_view) in enumerate(view_list):
                for (v2, gallery_view) in enumerate(view_list):
                    ...
                    gallery_x = feature[gseq_mask, :] #[100, 93, 256]
                    gallery_y = label[gseq_mask]

                    pseq_mask = np.isin(seq_type, probe_seq) & np.isin(view, [probe_view])
                    probe_x = feature[pseq_mask, :] #[200, 93, 256]
                    probe_y = label[pseq_mask]
                    total_dist = 0
                    num_part = probe_x.shape[1]
                    for i in range(num_part):
                        temp_probe = probe_x[:, i, :]
                        temp_gallery = gallery_x[:, i, :]
                        total_dist += cuda_dist(temp_probe, temp_gallery)
                    dist = total_dist / num_part
