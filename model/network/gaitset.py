...
class SetNet(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.part_feat_num = 2

        _branch_in_channels = 1
        _branch_channels = 32
        self.branch1 = SetBlock(BasicConv2d(_branch_in_channels, _branch_channels, 1))
        self.branch2 = SetBlock(BasicConv2d(_branch_in_channels, _branch_channels, 3, padding=1))
        self.branch3 = SetBlock(BasicConv2d(_branch_in_channels, _branch_channels, 5, padding=2))

        _set_in_channels = 1
        _set_channels = [_branch_channels*3, 128, 256]
        ...
        _gl_in_channels = _branch_channels*3
        ...
        self.part_feat = nn.ModuleList([BasicConv2d(_set_channels[0], _set_channels[1], 3, padding=1) \
                                                        for _ in range(self.part_feat_num)])
        self.conv3x3 = BasicConv2d(_set_channels[1], _set_channels[1], 3, stride=2, padding=1)
        self.conv1x1_1 = BasicConv2d(_set_channels[1], _gl_channels[1], 1)
        self.conv1x1_2 = BasicConv2d(_gl_channels[1], _set_channels[1], 1)
    ...
    def forward(self, silho, batch_frame=None):
        ...
        h = x.size(-2)
        temp_feat_list = []
        for i in range(self.part_feat_num):
            temp_feat = self.frame_max(x)[0][:, :, int(i*h/self.part_feat_num):int((i+1)*h/self.part_feat_num), :]
            temp_feat_list.append(self.part_feat[i](temp_feat))
        feat = torch.cat(temp_feat_list, -2)
        conv3x3 = self.conv3x3(feat)
        ...
        conv1x1 = self.conv1x1_1(self.frame_max(x)[0])
        ...
        conv1x1 = gl + conv1x1
        fuse_feat = torch.cat((conv3x3, self.conv1x1_2(conv1x1)), 1)
